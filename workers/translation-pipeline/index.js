  ...
  async run(event, step) {
    ...
    const phrases = await step.do('segment-phrases', async () => {
      const res = await callAnthropic({
        apiKey,
        system: `You are an expert in ${langName} text segmentation.`,
        messages: [{ role: 'user', content: `Identify clause boundaries, prepositional phrases, and verbal constructions in this ${langName} text. Break it into individual phrases (clause-level units).\n\nReturn JSON: { "phrases": ["phrase1", "phrase2", ...] }\n\nText:\n${job.sourceText}` }],
        json: true,
      });
    ...
    const sentences = await step.do('segment-sentences', async () => {
      const res = await callAnthropic({
        apiKey,
        system: `You are an expert in ${langName} text segmentation.`,
        messages: [{ role: 'user', content: `Group these phrases into complete sentences (semantic statements).\n\nReturn JSON: { "sentences": [{ "phrases": ["p1","p2"], "text": "full sentence" }, ...] }\n\nPhrases: ${JSON.stringify(phrases)}` }],
        json: true,
      });
    ...
    const segmented = await step.do('segment-join', async () => {
      const res = await callAnthropic({
        apiKey,
        system: `You are an expert in ${langName} text segmentation.`,
        messages: [{ role: 'user', content: `Validate paragraph boundaries and produce the final segmented structure.\n\nReturn JSON: { "paragraphs": [{ "text": "full paragraph", "sentences": [{ "text": "sentence", "phrases": ["p1","p2"] }] }] }\n\nSentences: ${JSON.stringify(sentences)}\nParagraph groupings: ${JSON.stringify(paras)}` }],
        json: true,
      });
